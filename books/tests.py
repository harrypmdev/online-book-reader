...
class TestAddBookView(TestCase):
    ...
    def test_render_valid_add_book_page(self):
        ...
        self.assertEqual(response.status_code, 200, msg="Add book page not returned.")
        self.assertIn(
            b"Add a new book",
            response.content,
        ...
        self.assertIn(
            b"input", response.content, msg="Add book page did not render URL form."
        )
    ...
    def test_invalid_add_book_page(self):
        ...
        self.assertRedirects(
            response, "/login/", msg_prefix="Did not redirect to login."
        )
...
class TestHomeAndDashboardView(TestCase):
    ...
    def test_book_appears_on_dashboard(self):
        ...
        self.assertEqual(response.status_code, 200, msg="Dashboard page not returned.")
        self.assertIn(
            b"Bill Porter",
            response.content,
    ...
    def test_home_page_unauthenticated(self):
        ...
        """
        response = self.client.get(reverse("home"))
        self.assertEqual(response.status_code, 200, msg="Home page not returned.")
        msg = "Index page did not return basic home " + "page when user not logged in."
...
class TestManageBookView(TestCase):
    ...
    def test_render_valid_manage_book_page(self):
        ...
        """
        self.client.login(username="existingUser", password="userPassword123%")
        response = self.client.get(reverse("manage_book", args=[self.user_book.id]))
        self.assertEqual(
            response.status_code, 200, msg="Manage book page not returned."
        )
        self.assertIn(
            b"Bill Porter",
            response.content,
    ...
    def test_invalid_manage_book_page(self):
        ...
        current user's book.
        """
        response = self.client.get(reverse("manage_book", args=[25]))
        msg = "Did not redirect to home/dashboard page."
        self.assertRedirects(response, "/", msg_prefix=msg)
...
class TestDeleteBookView(TestCase):
    ...
    def test_valid_delete_book(self):
        ...
        """
        self.client.login(username="existingUser", password="userPassword123%")
        response = self.client.get(reverse("delete_book", args=[self.user_book.id]))
        msg = "Did not redirect to home/dashboard page."
        self.assertRedirects(response, "/", msg_prefix=msg)
        ...
        self.assertFalse(book_exists, msg="Book still exists.")
